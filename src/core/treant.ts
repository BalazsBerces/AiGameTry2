 ...
 * Whenever the player comes close it also sweeps its branches, standing still for that too.
 */
...
export type TreantEvent = { kind: 'podsLand'; pods: SeedPod[] };
...
export const createTreant = (time: number): Treant => ({ nextAttackAt: time, throwSeedsNext: false, nextSweepAt: time });
...
export function updateTreant(treant: Treant, input: TreantInput): TreantStep {
  ...
  const next = { ...treant, attack, nextAttackAt, throwSeedsNext, sweep, nextSweepAt };
