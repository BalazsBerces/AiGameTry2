 ...
 * column. Zombies wait in the crushers' paths, to be lured under them. Crusher columns stay
...
const crusherCorridor: Archetype = {
  ...
  build({ width, height, rng }) {
    ...
    const enemies = zombies([{ x: lured, y: 3 }, { x: width - 1 - lured, y: 3 }]);
    if (rng.next() < 0.4) enemies.push(...[{ x: 0, y: 0 }, { x: width - 1, y: height - 1 }].map((cell): EnemySpawn => ({ type: 'turret', cell })));
    return {
      tiles: canvas.tiles,
      enemies,
      pickups: [],
