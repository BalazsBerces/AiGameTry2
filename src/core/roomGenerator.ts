...
export type EnemyType = 'zombie' | 'turret' | 'worm' | 'wormBoss' | 'shadowBoss' | 'treantBoss';
...
export function generateRoom(spec: RoomSpec, floorIndex: number, rng: Rng): RoomLayout {
  ...
  if (archetypesFor(floorIndex, spec.kind).length) {
    ...
    return { id: spec.id, width, height, tiles: built.tiles, doors, enemies: built.enemies, pickups, archetype: built.archetype };
  }
