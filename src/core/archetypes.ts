...
/**
 * Floor 1: thorn hedges wind through the room, mirrored into all four quadrants, with zombies
...
const thornMaze: Archetype = {
  ...
  build({ width, height, rng }) {
    const axes: MirrorAxis[] = ['vertical', 'horizontal'];
    const canvas = new Canvas(width, height, axes);
    // Each layout is a top-left quarter of hedge plus a lane cell a zombie prowls.
    const layout = rng.pick([
    ...
    return { tiles: canvas.tiles, enemies: zombies(chosen), pickups: [], symmetry: { axes } };
  },
};
