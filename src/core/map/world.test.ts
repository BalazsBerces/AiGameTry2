...
import { validateRoom } from '../rooms/roomValidator';
import {
  burstGlowshroom,
  createWorld,
  detonateBomb,
...
import { archetypeById, supportsShape } from '../rooms/archetypes';
import { CELL_TILES, PASSIVE_POOL, roomPadding } from '../rooms/roomGenerator';
...
describe('the caves cast', () => {
  it('fills floor 2 rooms with ghouls, crystal turrets, worms, bats and slimes only', () => {
    ...
    expect([...types].sort()).toEqual(['bat', 'crystalTurret', 'ghoul', 'slime', 'worm']);
  });
});
