...
| 2 (Caves) | **Worm boss**: a long worm that tunnels through the walls and out across the room, rampages in bursts of straight lunges, splits once into two halves, and lays eggs in its second phase. |
...
- **`src/core/`** holds the game's rules as plain, framework-free TypeScript: floor and room generation, enemy and boss decisions, the weapon model, items and the world state. It's all deterministic for a given seed and covered by unit tests beside each module.
- **`src/game/`** is the [Phaser 3](https://phaser.io/) layer. It handles physics, drawing and input, and asks `core` what should happen.
